...
if not release:
    ext_options["define_macros"].append(("CYTHON_TRACE", "1"))
...
if RTDFLAG:
    ...
    try:
        ...
        cykdtree_cpp = os.path.join(os.path.dirname(cykdtree.__file__), "c_kdtree.cpp")
...
def add_delaunay(ext_modules, src_include, ver, periodic=False, parallel=False, dont_compile=False):
    ext_name = _delaunay_filename("ext", ver, periodic=periodic, parallel=parallel)
    pyx_file = _delaunay_filename("pyx", ver, periodic=periodic, parallel=parallel)
    pxd_file = _delaunay_filename("pxd", ver, periodic=periodic, parallel=parallel)
    cpp_file = _delaunay_filename("cpp", ver, periodic=periodic, parallel=parallel)
    hpp_file = _delaunay_filename("hpp", ver, periodic=periodic, parallel=parallel)
    if not os.path.isfile(pyx_file):
        print("Extension {} ".format(ext_name) + "does not exist and will not be compiled")
        return
    ...
    if not dont_compile:
        if parallel:
            ...
            ext_modules.append(Extension(ext_name, sources=[pyx_file, cpp_file], **ext_options_cgal))
...
add_delaunay(ext_modules, src_include, "D", parallel=True, dont_compile=(not compile_parallel))

# Add other packages
ext_modules += [
    Extension("cgal4py.delaunay.tools", sources=["cgal4py/delaunay/tools.pyx"], **ext_options),
]
src_include += ["cgal4py/delaunay/tools.pyx", "cgal4py/delaunay/tools.pxd", "cgal4py/delaunay/c_tools.hpp"]
...
setup(
    name="cgal4py",
    packages=["cgal4py", "cgal4py.delaunay", "cgal4py.domain_decomp", "cgal4py.tests"],
    # package_dir = {'cgal4py':'cgal4py'}, # maybe comment this out
    package_data={"cgal4py": ["README.md", "README.rst"], "cgal4py.delaunay": src_include},
    version="0.2.1",
    ...
    keywords=["delaunay", "voronoi", "cgal", "triangulation", "tessellation"],
    classifiers=[
